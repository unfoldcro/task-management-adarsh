...
/* ══════════════════════════════════════════════════════════
   PERMISSION SYSTEM  (inspired by ClickUp / Monday.com)
   ══════════════════════════════════════════════════════════
...
const canEditTask = (user, task) => perm(user, "editAnyTask")   || task.assignee === user.id || task.createdBy === user.id;
const canDelTask  = (user, task) => perm(user, "deleteAnyTask") || task.createdBy === user.id;

const ROLE_BADGE = {
  Admin:     { bg: "#1A1A1A", fg: "#fff" },
  Manager:   { bg: "#6B4CE6", fg: "#fff" },
  Developer: { bg: "#1A7FE6", fg: "#fff" },
  Designer:  { bg: "#E6651A", fg: "#fff" },
  Marketing: { bg: "#2D8C3C", fg: "#fff" },
  QA:        { bg: "#C48A00", fg: "#fff" },
  Viewer:    { bg: "#AAA",    fg: "#fff" },
};
...
const DEMO_PROJECTS = [
  { id:"p1", name:"Website Redesign", client:"Acme Corp",     status:"active",    color:"#6B4CE6", members:["u1","u3"],        due:"2026-03-31", description:"Full redesign of Acme Corp website with CRO optimisation" },
  { id:"p2", name:"Mobile App v2",    client:"TechStart Inc", status:"active",    color:"#1A7FE6", members:["u1","u2","u5"],   due:"2026-04-15", description:"Version 2 of the TechStart mobile application" },
  { id:"p3", name:"Q1 Campaign",      client:"GrowthCo",      status:"active",    color:"#E6651A", members:["u1","u4"],        due:"2026-03-01", description:"Q1 marketing campaign for GrowthCo" },
  { id:"p4", name:"API Integration",  client:"DataFlow Ltd",  status:"completed", color:"#2D8C3C", members:["u1","u2","u5"],   due:"2026-02-25", description:"REST API integration for DataFlow platform" },
];

const DEMO_TASKS = [
  { id:uid(), title:"Redesign landing page hero section",   description:"Update hero with new CRO patterns",              assignee:"u3", priority:"high",   status:"in-progress", due:"2026-02-28", createdBy:"u1", project:"p1" },
  { id:uid(), title:"Fix authentication token refresh bug", description:"Token expires prematurely on mobile",             assignee:"u2", priority:"high",   status:"todo",        due:"2026-02-27", createdBy:"u1", project:"p2" },
  { id:uid(), title:"Write Q1 marketing report",            description:"Include conversion metrics and A/B test results", assignee:"u4", priority:"medium", status:"in-progress", due:"2026-03-01", createdBy:"u1", project:"p3" },
  { id:uid(), title:"Set up CI/CD pipeline for staging",    description:"Automate deployment to staging env",              assignee:"u5", priority:"medium", status:"done",        due:"2026-02-25", createdBy:"u1", project:"p2" },
  { id:uid(), title:"User research interviews — batch 2",   description:"5 interviews with enterprise users",              assignee:"u3", priority:"low",    status:"todo",        due:"2026-03-05", createdBy:"u1", project:"p1" },
  { id:uid(), title:"API rate limiting implementation",     description:"Add rate limiting middleware",                    assignee:"u2", priority:"high",   status:"todo",        due:"2026-03-02", createdBy:"u1", project:"p4" },
  { id:uid(), title:"Social media content calendar",        description:"Plan March social content",                       assignee:"u4", priority:"low",    status:"done",        due:"2026-02-24", createdBy:"u1", project:"p3" },
  { id:uid(), title:"Database migration script review",     description:"Review and test migration scripts",               assignee:"u5", priority:"medium", status:"in-progress", due:"2026-02-28", createdBy:"u1", project:"p4" },
];
...
const PROJ_STATUS_CFG = {
  active:    { label:"Active",    bg:"#EEFBE6", fg:"#2D8C3C" },
  completed: { label:"Completed", bg:"#F5F5F5", fg:"#888"    },
  on_hold:   { label:"On Hold",   bg:"#FFF8E6", fg:"#C48A00" },
};
  ...
  return (
    <div style={css.shell}>
      ...
      <div style={css.content}>
        {view === "dashboard" && <Dashboard tasks={tasks} team={team} user={user} projects={projects} />}
        {view === "tasks"     && (
          <Tasks
            tasks={tasks} setTasks={setTasks} team={team} user={user}
            setModal={setModal} projects={projects}
            fMember={fMember} setFMember={setFMember}
            fStatus={fStatus} setFStatus={setFStatus}
            fProject={fProject} setFProject={setFProject}
          />
        )}
        {view === "projects" && <Projects projects={projects} setProjects={setProjects} tasks={tasks} team={team} user={user} setModal={setModal} />}
      ...
      {modal && (
        <Modal
          ...
          user={user}   projects={projects} setProjects={setProjects}
        />
      )}
    </div>
  );
    ...
    <div style={css.loginWrap}>
      ...
      <div style={css.loginLeft}>
        <div style={css.loginLeftInner}>
          ...
          <p style={css.loginP}>Assign tasks, track client projects, and manage access — one platform for your entire workflow.</p>

          <div style={css.featureList}>
            {["Real-time task tracking","Client project management","Role-based access control","Team workload overview"].map((f,i) => (
    ...
    <div className="anim-up" style={css.page}>
      <div style={css.pgHead}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ ...css.pgDate, background:"#EEFBE6", border:"1px solid #B8E986", color:"#2D8C3C" }}>{activeProjects} active project{activeProjects!==1&&"s"}</div>
      ...
      <div style={css.progressCard}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <span style={{ fontSize:14, color:"#888", fontWeight:500 }}>Overall Task Completion</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:14, marginBottom:40 }}>
        {projects.filter(p=>p.status==="active").map((p,i) => {
          ...
          return (
            <div key={p.id} className="anim-up stat-card" style={{ ...css.wlCard, animationDelay:`${i*0.06}s`, borderTop:`3px solid ${p.color}` }}>
              <div style={{ fontSize:11, color:p.color, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:4 }}>{p.client}</div>
              <div style={{ fontSize:15, fontWeight:700, color:"#1A1A1A", marginBottom:14 }}>{p.name}</div>
              <div style={css.progTrack}>
                <div style={{ height:"100%", width:`${pp}%`, background:p.color, borderRadius:8, transition:"width 0.5s" }} />
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", marginTop:8, fontSize:12, color:"#AAA" }}>
                <span>{done}/{pt.length} tasks</span>
                <span style={{ fontWeight:700, color:p.color }}>{pp}%</span>
              </div>
            </div>
          );
        })}
      ...
      <div style={css.wlGrid}>
        {team.map((m,i) => {
          ...
          const p2 = mt.length ? Math.round((dn/mt.length)*100) : 0;
            ...
            <div key={m.id} className="anim-up stat-card" style={{ ...css.wlCard, animationDelay:`${i*0.06}s` }}>
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize:11, color:rb.fg, background:rb.bg, padding:"2px 7px", borderRadius:4, fontWeight:700, letterSpacing:"0.04em" }}>{m.role}</span>
                </div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                <div style={css.progTrackSm}>
                  <div style={{ height:"100%", width:`${p2}%`, background:m.color, borderRadius:4, transition:"width 0.5s ease" }} />
                </div>
                <span style={{ fontSize:13, color:"#1A1A1A", fontWeight:700, minWidth:36, textAlign:"right" }}>{p2}%</span>
              </div>
              <div style={{ display:"flex", gap:16, fontSize:12, color:"#999" }}>
                <span><span style={{ width:6,height:6,borderRadius:"50%",background:"#CCC",display:"inline-block",marginRight:4 }} />{mt.length-dn-ip} todo</span>
                <span><span style={{ width:6,height:6,borderRadius:"50%",background:"#E6A516",display:"inline-block",marginRight:4 }} />{ip} active</span>
                <span><span style={{ width:6,height:6,borderRadius:"50%",background:"#2D8C3C",display:"inline-block",marginRight:4 }} />{dn} done</span>
              </div>
            </div>
          );
        })}
...
function Tasks({ tasks, setTasks, team, user, setModal, projects, fMember, setFMember, fStatus, setFStatus, fProject, setFProject }) {
  ...
  const del = (id, task) => {
    if (!canDelTask(user, task)) return;
    ...
    <div className="anim-up" style={css.page}>
      ...
      <div style={css.tblWrap}>
        ...
        {filtered.map(t => {
          ...
          const canDel  = canDelTask(user, t);
          return (
            <div key={t.id} className="tbl-row" style={css.tblRow}>
              ...
              <div style={{ width:120, minWidth:0 }}>
                {proj && (
                  <span style={{ fontSize:11, fontWeight:700, color:proj.color, background:`${proj.color}18`, padding:"3px 8px", borderRadius:4, display:"block", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                    {proj.name}
                  </span>
                )}
              </div>
              ...
              <div style={{ width:100 }}>
                <button
                  onClick={() => cycle(t.id, t)}
                  className="st-btn"
                  style={{ ...css.stBtn, background:st.bg, color:st.fg, borderColor:st.ring, cursor:canEdit?"pointer":"default", opacity:canEdit?1:0.65 }}
                  title={canEdit ? "Click to advance status" : "No permission to change status"}
                >
                  {st.label}
                </button>
              </div>
              ...
              <div style={{ width:36 }}>
                {canDel && <button onClick={() => del(t.id, t)} className="x-btn" style={css.xBtn}>×</button>}
              </div>
            </div>
          );
        })}
      ...
      {/* Permission hint */}
      {!perm(user,"editAnyTask") && (
        <p style={{ marginTop:14, fontSize:12, color:"#BBB" }}>
          ℹ You can update status / delete tasks that are assigned to you or created by you.
        </p>
      )}
    </div>
  );
    ...
    <div className="anim-up" style={css.page}>
      <div style={css.pgHead}>
        ...
        {perm(user,"manageProjects") && (
          <button onClick={() => setModal({ type:"new-project" })} className="btn-primary" style={css.btnPrimary}>+ New Project</button>
        )}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))", gap:16 }}>
        {projects.map((p,i) => {
          ...
          const pp      = pt.length ? Math.round((done/pt.length)*100) : 0;
          const members = p.members.map(id=>team.find(m=>m.id===id)).filter(Boolean);
          const ps      = PROJ_STATUS_CFG[p.status] || PROJ_STATUS_CFG.active;
          return (
            <div key={p.id} className="anim-up stat-card" style={{ ...css.tmCard, animationDelay:`${i*0.06}s`, borderTop:`3px solid ${p.color}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                <div>
                  <div style={{ fontSize:11, color:p.color, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:4 }}>{p.client}</div>
                  <div style={{ fontSize:17, fontWeight:800, color:"#1A1A1A" }}>{p.name}</div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <span style={{ fontSize:11, fontWeight:700, color:ps.fg, background:ps.bg, padding:"3px 10px", borderRadius:6 }}>{ps.label}</span>
                  {perm(user,"manageProjects") && (
                    <button onClick={() => del(p.id)} className="x-btn" style={css.xBtn}>×</button>
                  )}
                </div>
              </div>
              {p.description && <p style={{ fontSize:13, color:"#AAA", marginBottom:16, lineHeight:1.5 }}>{p.description}</p>}
              <div style={css.progTrack}>
                <div style={{ height:"100%", width:`${pp}%`, background:p.color, borderRadius:8, transition:"width 0.5s" }} />
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", marginTop:8, marginBottom:16, fontSize:12, color:"#AAA" }}>
                <span>{done} of {pt.length} tasks done</span>
                <span style={{ fontWeight:700, color:p.color }}>{pp}%</span>
              </div>
              <div style={{ display:"flex", gap:18, marginBottom:16, fontSize:12 }}>
                <span style={{ color:"#888" }}><b style={{ color:"#1A1A1A" }}>{todo}</b> todo</span>
                <span style={{ color:"#888" }}><b style={{ color:"#C48A00" }}>{inprog}</b> active</span>
                <span style={{ color:"#888" }}><b style={{ color:"#2D8C3C" }}>{done}</b> done</span>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingTop:14, borderTop:"1px solid #F5F5F5" }}>
                <div style={{ display:"flex" }}>
                  {members.map((m,j) => (
                    <div key={m.id} title={m.name} style={{ ...css.av32, background:m.color, color:"#fff", marginLeft: j>0?-8:0, border:"2px solid #fff", zIndex:j, position:"relative" }}>{m.avatar}</div>
                  ))}
                </div>
                <div style={{ fontSize:11, color:"#AAA" }}>Due {new Date(p.due).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</div>
              </div>
            </div>
          );
        })}
...
function Team({ team, setTeam, tasks, user, setModal }) {
  const rm = (id) => {
    if (!perm(user,"manageTeam") || id===user.id) return;
  ...
  return (
    <div className="anim-up" style={css.page}>
      ...
      {!perm(user,"manageTeam") && (
        <div style={css.infoBanner}>
          <span>ℹ</span>
          <span>You have <b>{user.role}</b> access. Contact an Admin or Manager to manage team members or change roles.</span>
        </div>
      )}
      ...
      <div style={css.tmGrid}>
        {team.map((m,i) => {
          ...
          const rb   = ROLE_BADGE[m.role] || ROLE_BADGE.Viewer;
          const rp   = ROLE_PERMISSIONS[m.role] || ROLE_PERMISSIONS.Viewer;
          return (
            <div key={m.id} className="anim-up stat-card" style={{ ...css.tmCard, animationDelay:`${i*0.06}s` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ display:"flex", gap:14, alignItems:"center" }}>
                  ...
                  <div>
                    ...
                    <span style={{ fontSize:11, color:rb.fg, background:rb.bg, padding:"2px 9px", borderRadius:4, fontWeight:700, letterSpacing:"0.04em", display:"inline-block", marginTop:3 }}>{m.role}</span>
                    <div style={{ fontSize:12, color:"#CCC", marginTop:4 }}>{m.email}</div>
                  </div>
                </div>
                {perm(user,"manageTeam") && m.id!==user.id && (
                  <button onClick={() => rm(m.id)} className="x-btn" style={css.xBtn}>×</button>
                )}
              </div>
              <div style={css.tmStats}>
                <div style={css.tmStat}><span style={css.tmStatV}>{mt.length}</span><span style={css.tmStatL}>TASKS</span></div>
                <div style={css.tmStat}><span style={css.tmStatV}>{dn}</span><span style={css.tmStatL}>DONE</span></div>
                <div style={css.tmStat}><span style={{ ...css.tmStatV, color:rate>0?"#2D8C3C":"#CCC" }}>{rate}%</span><span style={css.tmStatL}>RATE</span></div>
              </div>
              ...
              <div style={{ marginTop:14, paddingTop:14, borderTop:"1px solid #F5F5F5", display:"flex", flexWrap:"wrap", gap:6 }}>
                {[
                  ...
                  <span key={key} style={{ fontSize:10, padding:"2px 8px", borderRadius:4, background:rp[key]?"#EEFBE6":"#F5F5F5", color:rp[key]?"#2D8C3C":"#BBB", fontWeight:600 }}>
                    {rp[key]?"✓ ":""}{label}
                  </span>
                ))}
              </div>
...
/* ════════════════════════ MODAL ════════════════════════ */
function Modal({ modal, close, tasks, setTasks, team, setTeam, user, projects, setProjects }) {
  /* Task form */
  ...
  const [taskProj, setTaskProj] = useState(projects[0]?.id || "");
  ...
  const [pjDue,     setPjDue]     = useState("2026-03-31");
  const [pjStatus,  setPjStatus]  = useState("active");
  const [pjMembers, setPjMembers] = useState([user.id]);

  const addTask = () => {
    if (!title.trim()) return;
    setTasks(p => [{ id:uid(), title:title.trim(), description:desc.trim(), assignee, priority:prio, status:"todo", due, createdBy:user.id, project:taskProj }, ...p]);
    close();
  };
  ...
  const addProject = () => {
    if (!pjName.trim() || !pjClient.trim()) return;
    const cols = ["#6B4CE6","#1A7FE6","#E6651A","#2D8C3C","#DC2626","#C48A00","#0EA5E9"];
    setProjects(p => [...p, { id:uid(), name:pjName.trim(), client:pjClient.trim(), status:pjStatus, color:cols[Math.floor(Math.random()*cols.length)], members:pjMembers, due:pjDue, description:pjDesc.trim() }]);
    close();
  };

  const toggleMember = (id) => setPjMembers(prev => prev.includes(id) ? prev.filter(x=>x!==id) : [...prev,id]);
    ...
    <div style={css.overlay} onClick={close}>
      <div className="anim-up" style={css.modal} onClick={e=>e.stopPropagation()}>
        ...
        {modal.type==="new-task" && (
          <>
            ...
            <div style={{ display:"flex", gap:14, marginTop:16 }}>
              <div style={{ flex:1 }}>
                <label style={css.lbl}>PROJECT</label>
                <select value={taskProj} onChange={e=>setTaskProj(e.target.value)} style={css.inp}>
            ...
            <div style={{ display:"flex", gap:14, marginTop:16 }}>
              ...
              <div style={{ flex:1 }}>
                <label style={css.lbl}>DUE DATE</label>
        ...
        {modal.type==="add-member" && (
          <>
            ...
            <select value={mRole} onChange={e=>setMRole(e.target.value)} style={css.inp}>
              {["Admin","Manager","Developer","Designer","Marketing","QA","Viewer"].map(r=><option key={r}>{r}</option>)}
            </select>
            ...
            <div style={{ marginTop:14, background:"#F7F7F7", borderRadius:8, padding:"12px 14px" }}>
              ...
              {[
                ...
                return (
                  <div key={key} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, fontSize:12, color:has?"#2D8C3C":"#CCC" }}>
        ...
        {modal.type==="new-project" && (
          <>
            <h3 style={css.mTitle}>New Project</h3>
            <div style={{ display:"flex", gap:14 }}>
              <div style={{ flex:1 }}>
                <label style={css.lbl}>PROJECT NAME</label>
                <input value={pjName} onChange={e=>setPjName(e.target.value)} placeholder="Website Redesign" style={css.inp} autoFocus />
              </div>
              <div style={{ flex:1 }}>
                <label style={css.lbl}>CLIENT</label>
                <input value={pjClient} onChange={e=>setPjClient(e.target.value)} placeholder="Acme Corp" style={css.inp} />
              </div>
            </div>
            <label style={{ ...css.lbl, marginTop:16 }}>DESCRIPTION</label>
            <input value={pjDesc} onChange={e=>setPjDesc(e.target.value)} placeholder="Brief project overview (optional)" style={css.inp} />
            <div style={{ display:"flex", gap:14, marginTop:16 }}>
              <div style={{ flex:1 }}>
                <label style={css.lbl}>STATUS</label>
                <select value={pjStatus} onChange={e=>setPjStatus(e.target.value)} style={css.inp}>
              ...
              <div style={{ flex:1 }}>
                <label style={css.lbl}>DUE DATE</label>
                <input type="date" value={pjDue} onChange={e=>setPjDue(e.target.value)} style={css.inp} />
              </div>
            </div>
            <label style={{ ...css.lbl, marginTop:16 }}>TEAM MEMBERS</label>
              ...
              {team.map(m => (
                <button
                  ...
                  style={{
                    ...
                    border:`1px solid ${pjMembers.includes(m.id) ? m.color : "#EBEBEB"}`,
                    background: pjMembers.includes(m.id) ? `${m.color}15` : "#fff",
                    cursor:"pointer", fontSize:12, fontWeight:600,
                    color: pjMembers.includes(m.id) ? m.color : "#999",
                    transition:"all 0.15s",
                  }}
                >
                  <div style={{ ...css.av22, background:m.color, color:"#fff", flexShrink:0 }}>{m.avatar}</div>
                  {m.name.split(" ")[0]}
                </button>
              ))}
            </div>
            <div style={css.mActions}>
              <button onClick={close} style={css.btnGhost}>Cancel</button>
              <button onClick={addProject} className="btn-primary" style={css.btnPrimary}>Create Project</button>
            </div>
          </>
        )}
      </div>
...
const css = {
  ...
  modal: { background:"#fff", border:"1px solid #EBEBEB", borderRadius:20, padding:32, width:"100%", maxWidth:520, boxShadow:"0 24px 48px rgba(0,0,0,0.12)", maxHeight:"90vh", overflowY:"auto" },
  ...
  /* Info banner */
  infoBanner: { background:"#FFF8E6", border:"1px solid #FFD875", borderRadius:10, padding:"12px 18px", marginBottom:24, fontSize:13, color:"#C48A00", display:"flex", alignItems:"center", gap:10 },
  ...
  credCode: { fontSize:13, color:"#999", fontFamily:"var(--mono)", lineHeight:1.8 },
